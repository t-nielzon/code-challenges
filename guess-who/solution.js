...
class GuessWho {
  constructor(character) {
    ...
    this.turns = 0;
    this.possible = characteristics.map(char => char[0]);
  }
  ...
  guess(g) {
    ...
    const charData = characteristics.find(char => char[0] === this.character);
    const hasCharacteristic = charData[1].includes(g);
    
    if (hasCharacteristic) {
      this.possible = this.possible.filter(charName => {
        const data = characteristics.find(c => c[0] === charName);
        return data[1].includes(g);
      });
    } else {
      this.possible = this.possible.filter(charName => {
        const data = characteristics.find(c => c[0] === charName);
