 ...
 * all having the same design each in `2` colors. Posters must all have a different color combination
 * and you have the choice of `4` colors: `red, blue, yellow, green`.
 ...
 * Fortunately if you take `7 choose 3` or `7 choose 4` combinations you get `35` and so each poster will have
 * a different combination of `3` colors or `4` colors. You will take `3` colors because it's less expensive.
 *
 * Hence the problem:
 *
 * Knowing `m` (number of posters to design) and `n` (total number of available colors),
 * let us search `x`, the number of colors for each poster so that each poster has a unique combination
 * of colors and the number of combinations is exactly the same as the number of posters `m`.
 *
