/*
 * ASCII Games: Flood Fill (Player)
 * 5 kyu
 *
 * You will be given a rectangular map with characters 1-5 representing different colors.
 * During every turn, you can choose a coordinate x,y (top left origin) and a color 1-5,
 * which will flood-fill every orthogonally adjacent tile of the same color as the chosen
 * tile into the new color.
 *
 * Play back the moves and return the result of the transformation.
 *
 * Moves are in the form: [{x, y, color}, ...]
 */
function floodFillGame(board, moves) {
}