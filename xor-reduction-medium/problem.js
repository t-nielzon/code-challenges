/*
Xor reduction (medium)

Given two integers, m and n, return the cumulative xor of all positive integers 
between them, inclusive. E.g. (0, 6), return `0^1^2^3^4^5^6` => `7`.

Constraints:
- 0 <= m < n <= 10^15

Hints:
- Try the easy variant first
- Brute-force solutions will likely be rejected due to time-out
- Read more about xor on Wikipedia
- Look for patterns for successive n values
*/

function xorReduction(m, n) {
  
}