...
where the t(i) are the substrings of s;
...
length s(0) <= length s(1) >= length s(2) <= length s(3) >= length s(4) <= length s(5)
...
When you have property (P), to make the result more "up and down" visible
t(0), t(2), ... will be lower cases and the others upper cases.
