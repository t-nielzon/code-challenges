...
//     (generated at time t by pedaling and measured in watts) at the start of the trip is
//     WATTS0 = 225 (watts)
//   - We don't take account of the rolling resistance
//   - When he starts climbing at t = 0 his initial speed (pushed start) is v0 (km/h)
//   - His initial acceleration gamma is 0. gamma is in km/h/min at time t. It is the number of
//     kilometers per hour he gains or loses in the next minute.
...
//        (slope is a percentage, not an angle. You have to determine function(slope).)
//     2) air drag is - DRAG * abs(v) * abs(v) / MASS where v is his current speed
//     3) if his power and his speed are both strictly positive we add the thrust (by pedaling):
...
//   slope // ascent in percentage (don't forget to divide by 100 when needed)
//
// Task:
//   Write function temps(v0, slope, dTot) which returns as a rounded integer the time t in
//   minutes needed to travel dTot. If John gives up return -1.
//
// Reminders:
//   1) speed at (t + DELTA_T) = (speed at t) + gamma * DELTA_T
//   2) distance at (t + DELTA_T) = (distance at t) + speed * DELTA_T / 60.0
//      where speed is calculated with 1)
//
// Examples:
//   temps(30, 5, 30)  -> 114
//   temps(30, 20, 30) -> -1
